 ...
 */
public class ManageExcludeTexts extends ManageListAbstract {
     ...
     */
    public ManageExcludeTexts(IConfigurationControler configurationControler) {
        super(ConfigurationUtils.getInstance().getDisplayMessage(Constants.WINDOW_MANAGE_EXCLUDE_TEXTS_PANEL_TITLE),
        ...
        this.informationPanel = new InformationPanel(PictureTypeEnum.INFORMATION,
                ConfigurationUtils.getInstance()
                        .getDisplayMessage(Constants.WINDOW_MANAGE_EXCLUDE_TEXTS_INFORMATION_LABEL),
                ConfigurationUtils.getInstance()
                        .getDisplayMessage(Constants.WINDOW_MANAGE_EXCLUDE_TEXTS_INFORMATION_MESSAGE),
                false, true);
        createWindow();
    }
    ...
    private IProfileWithTable createProfileWithTable() {
        ProfilWithTableCmd profilWithTableCmd = new ProfilWithTableCmdBuilder()
                .titlePanel(ConfigurationUtils.getInstance()
                        .getDisplayMessage(Constants.WINDOW_MANAGE_EXCLUDE_TEXTS_PANEL_TITLE))
                .defaultProfile(getControler().getLexicometricDefaultProfile())
                .titleTablePanel(ConfigurationUtils.getInstance()
                        .getDisplayMessage(Constants.WINDOW_MANAGE_EXCLUDE_TEXTS_TABLE_PANEL_TITLE))
                .lexicometricConfiguration(getControler().getLexicometricConfiguration(LexicometricEditEnum.EXCLUDE_TEXTS, ExcludeTextsHierarchicalEditEnum.BASE))
                .lexicometricEditEnum(LexicometricEditEnum.EXCLUDE_TEXTS)
        ...
        profileWithTable.setInterfaceForTableAndAddButton(0,
                ConfigurationUtils.getInstance()
                        .getDisplayMessage(Constants.WINDOW_MANAGE_EXCLUDE_TEXTS_ADD_INFORMATION_MESSAGE),
                ConfigurationUtils.getInstance()
                        .getDisplayMessage(Constants.WINDOW_MANAGE_EXCLUDE_TEXTS_ADD_TEXT_LABEL));
        profileWithTable.setLabelForAddAndRemoveButton(0,
                ConfigurationUtils.getInstance()
                        .getDisplayMessage(Constants.WINDOW_MANAGE_EXCLUDE_TEXTS_ADD_BUTTON_LABEL),
                ConfigurationUtils.getInstance()
                        .getDisplayMessage(Constants.WINDOW_MANAGE_EXCLUDE_TEXTS_REMOVE_BUTTON_LABEL));
    ...
    public void initComponents() {
        ...
        this.content.add(getNewInstanceOfDefaultAction(LexicometricEditEnum.EXCLUDE_TEXTS).getJPanel());
    }
